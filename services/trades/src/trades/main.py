...
def setup_kafka(kafka_broker_address: str, kafka_topic: str) -> tuple[Application, TopicConfig]:
...
def process_historical_data(producer, topic: TopicConfig, kraken_api: KrakenRESTAPI) -> None:
    ...
    try:
        ...
        if not events:
            logger.warning("No trades were found. Check the time range and product IDs.")
            return
        ...
        for i, event in enumerate(events):
            ...
            if i % 100 == 0 or i == total - 1:
                ...
                sys.stdout.write(f"\rPublishing trades: {i+1}/{total} ({progress:.1f}%)...")
                sys.stdout.flush()
        ...
        logger.info(f"Successfully backfilled {len(events)} trades for {config.last_n_days} days")
...
def process_websocket_data(producer, topic: TopicConfig, kraken_api: KrakenWebSocketAPI) -> None:
...
def get_api_client() -> KrakenRESTAPI | KrakenWebSocketAPI:
    ...
    if config.kraken_api_mode == "REST":
        logger.info(f"Using Kraken REST API for historical data (last {config.last_n_days} days)")
        return KrakenRESTAPI(product_ids=config.product_ids, last_n_days=config.last_n_days)
